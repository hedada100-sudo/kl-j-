...
// --- 5. System Config Schema (Firebase, FAQ, Winning Percentage, SEO, Commission) ---
...
// ---------- 11. SYSTEM CONFIG (SEO, Firebase, FAQ, Winning Percentage, Commission) ----------
    ...
    try {
        ...
        if (!config) {
            ...
            if (key === 'commission_rate') {
                return res.json({ success: true, value: 60 });
            }
            if (key === 'seo_data') {
...
// 12.7 সাব-অ্যাডমিন ব্যালেন্স অ্যাড (কমিশন সহ) — মূল লজিক
app.post('/api/sub-admin/add-balance', async (req, res) => {
    try {
        ...
        // 1. কমিশন রেট fetch করুন (সার্ভার থেকে)
        ...
        // 5. সাব-অ্যাডমিনের ব্যালেন্স: পুরো টাকা কাটা হবে
        const beforeBalance = sub.balance;
        sub.balance -= amount; // পুরো টাকা কাটা
        await sub.save();

        // 6. কমিশন যোগ হবে সাব-অ্যাডমিনের ব্যালেন্সে (Net লস = amount - commission)
        // কিন্তু আমরা উপরে পুরো টাকা কেটেছি, এখন কমিশন যোগ করি
        sub.balance += commission;
        await sub.save();

        // 7. ইউজারের ব্যালেন্স বাড়ান (User receives)
        ...
        const log = new TransactionLog({
            ...
            userEmail: userEmail.toLowerCase(),
            amount: amount, // সাব-অ্যাডমিন যা পাঠিয়েছে (ফুল)
            commission: commission, // কমিশন যা সাব-অ্যাডমিন পাবে
            netChange: -(amount - commission), // নেট পরিবর্তন (যা ব্যালেন্স থেকে কাটা হয়েছে)
            beforeBalance: beforeBalance,
            afterBalance: sub.balance,
