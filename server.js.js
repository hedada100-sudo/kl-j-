...
const referralLogSchema = new mongoose.Schema({
    ...
    type: { type: String, enum: ['signup', 'deposit'], default: 'signup' },
    ...
    try {
        ...
        if (referrer) {
            try {
                ...
                console.log(`✅ Referral signup bonus: ${signupBonus} USDT to ${referrer.email} and ${newUser.email}`);
            } catch (err) {
...
// ============================================================
//  ৩. ADMIN: USERS, DEPOSIT, WITHDRAW, BALANCE, REQUESTS
// ============================================================
app.get('/api/admin/users', async (req, res) => {
    try {
        const users = await User.find({}, {
            email: 1, balance: 1, createdAt: 1, isGoogle: 1,
            referralCode: 1, referralEarnings: 1, referredBy: 1, _id: 1
        }).sort({ createdAt: -1 });
...
// ============================================================
//  ৪. APPROVE REQUEST (with Referral Bonus)
// ============================================================
                ...
                if (user.referredBy) {
                    try {
                        ...
                        if (referrer) {
                            ...
                            console.log(`✅ Referral deposit bonus: ${totalBonus} USDT (${depositBonus} fixed + ${depositPercent}% of ${reqDoc.amount}) to ${referrer.email}`);
                        }
                    } catch (err) {
...
// ============================================================
//  ৫. USER: DEPOSIT, WITHDRAW, TRADE, BALANCE
// ============================================================
...
app.post('/api/sub-admin/register', async (req, res) => {
    try {
        ...
        if (exists) return res.status(409).json({ success: false, message: 'Sub-admin already exists' });
...
app.get('/', (req, res) => {
    res.json({
        message: 'Global Trading API v5.0 (with Referral System)',
        endpoints: {
