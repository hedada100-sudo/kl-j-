...
.catch(err => {
    ...
    // MongoDB না থাকলেও সার্ভার চালু থাকবে (JSON ফাইল ব্যবহার করবে না, কিন্তু আমরা চালু রাখব)
    // কিন্তু আমরা MongoDB ছাড়া কাজ করব না – তাই exit করছি না, বরং error log করছি
});
...
// ============================================================
//  ৭. USER: DEPOSIT (FIXED)
// ============================================================
app.post('/api/user/deposit', async (req, res) => {
    try {
        const { userEmail, method, txid, amount } = req.body;
        console.log('Deposit request received:', { userEmail, method, txid, amount });
        ...
        // 3. Check if deposit method exists
        ...
        console.log('Deposit request saved:', newReq._id);
        res.json({ success: true, message: 'Deposit request submitted', requestId: newReq._id });
    } catch (error) {
        console.error('Deposit error:', error);
        res.status(500).json({ success: false, message: 'Server error during deposit submission: ' + error.message });
    }
});

// ============================================================
//  ৮. USER: WITHDRAW (FIXED)
// ============================================================
app.post('/api/user/withdraw', async (req, res) => {
    try {
        const { userEmail, method, address, amount } = req.body;
        console.log('Withdraw request received:', { userEmail, method, address, amount });
        ...
        // 3. Check if withdraw method exists
        ...
        console.log('Withdraw request saved:', newReq._id);
        res.json({ success: true, message: 'Withdraw request submitted', newBalance: user.balance, requestId: newReq._id });
    } catch (error) {
        console.error('Withdraw error:', error);
        res.status(500).json({ success: false, message: 'Server error during withdraw submission: ' + error.message });
    }
});
