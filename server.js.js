...
// --- 5. System Config Schema (Firebase, FAQ, Winning Percentage) ---
...
// 11.1 Get config by key (with default for winning_percentage)
app.get('/api/admin/config/:key', async (req, res) => {
    try {
        const { key } = req.params;
        const config = await Config.findOne({ key });
        if (!config) {
            // Return default for winning_percentage if not set
...
// 11.2 Set/Update config (upsert) - supports any key (firebase, faq, winning_percentage, etc.)
